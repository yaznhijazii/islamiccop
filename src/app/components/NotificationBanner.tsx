...
export function InteractiveTimeline() {
  ...
  useEffect(() => {
    const fetchPrayerTimes = async () => {
      try {
        ...
        const url = `https://api.aladhan.com/v1/timings/${dateString}?latitude=31.9454&longitude=35.9284&method=`;
  ...
  useEffect(() => {
    ...
    // Listen to storage events to sync with other components
  ...
  useEffect(() => {
    if (tasks.length > 0 && scrollContainerRef.current) {
      const activeTaskIndex = tasks.findIndex(
        (task) => task.isActive && !completionStatus[task.id],
      );
      if (activeTaskIndex !== -1) {
        const taskElements =
          scrollContainerRef.current.querySelectorAll("button");
  ...
  const updateTasks = () => {
    ...
    const allTasks: TimelineTask[] = [
      ...
      // Morning Athkar
      {
        ...
        timeInMinutes: 6 * 60,
        isActive:
          currentMinutes >= 6 * 60 && currentMinutes < 12 * 60,
      ...
      // Baqarah
      {
        ...
        timeInMinutes: 8 * 60,
        isActive:
          currentMinutes >= 8 * 60 && currentMinutes < 22 * 60,
      ...
      // Asr
      {
        ...
        isCompleted: false,
         ...
         // Evening Athkar - After Asr by 1 hour
      {
        id: "athkar-evening",
        time: prayerTimes.Asr,
        title: "أذكار المساء",
        ...
        timeInMinutes: asrMinutes + 60, // After Asr by 1 hour
        isActive:
          currentMinutes >= asrMinutes + 60 && currentMinutes < maghribMinutes,
        isPast: currentMinutes >= maghribMinutes,
        isCompleted: false,
      },
      // Maghrib
      {
  ...
  const handleToggleTask = (taskId: string) => {
    ...
    // Special handling for Quran tasks - reset progress if uncompleting
    if (task.type === 'quran' && task.storageField && completionStatus[taskId]) {
      // User is uncompleting a completed Quran task - reset the progress
      const progressKey = `quran-progress-${task.storageField}-${currentUsername}-${new Date().toDateString()}`;
      const progressData = {
        currentPage: 1,
        currentAyah: 0,
        totalPages: task.storageField === 'baqarah' ? 7 : 1,
        completed: false,
      };
  ...
  return (
    <div className="bg-gradient-to-br from-card/90 to-card/70 backdrop-blur-xl rounded-2xl p-4 sm:p-5 border border-border/50 shadow-lg sticky top-4">
      ...
      <div
        className="space-y-2.5 max-h-[calc(100vh-200px)] overflow-y-auto custom-scrollbar pr-1"
